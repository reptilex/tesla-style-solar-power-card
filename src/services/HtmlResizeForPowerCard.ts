/* eslint-disable prefer-template, no-param-reassign, class-methods-use-this, lit-a11y/click-events-have-key-events */
...
{
    public static changeStylesDependingOnWidth(teslaCard:TeslaStyleSolarPowerCard, solarCardElements:Map<string,SensorElement>, newWidth: number, oldWidth:number):number {
        ...
        const changeSelectorStyle = function(selector: string, styleAttribute:any, attributeValue:string) {
            const selectorElement = <HTMLElement > teslaCardElement.querySelector(selector)
            if (selectorElement !== null) selectorElement.style[styleAttribute] = attributeValue;
        }
    
        changeSelectorStyle('.acc_left', 'top',12 * pxRate +'px');
        changeSelectorStyle('.acc_right', 'top',12 * pxRate +'px');
        if(solarCardElements.get('battery_consumption_entity') === undefined 
          && solarCardElements.get('appliance2_consumption_entity') !== undefined ){
          changeSelectorStyle('.acc_center_container', 'margin-bottom', 15 * pxRate + 'px');
        }
    
        // icons
        teslaCardElement.querySelectorAll('.acc_container').forEach(
          (_currentValue, currentIndex, iconContainerItem) => {
        ...
        teslaCardElement.querySelectorAll('ha-icon').forEach(
          (_currentValue, currentIndex, icons) => {
        ...
        let selectorElement = <HTMLElement > teslaCardElement.querySelector(".power_lines svg")
        if (selectorElement !== null) selectorElement.setAttribute('viewBox','0 0 ' + 42 * pxRate + ' ' + 42 * pxRate);
        const half = 22 * pxRate;
        changeSelectorStyle('#generation_to_house_entity_line','d', 'M'+ half +',0 C'+ half +','+ half +' '+ half +','+ half +' '+half*2+','+half);
        changeSelectorStyle('#grid_feed_in_entity_line','d', 'M'+ half +',0 C'+ half +','+ half +' '+ half +','+ half +' 0,'+ half);
        changeSelectorStyle('#grid_to_house_entity_line','d',  'M0,'+half+' C'+half+','+ half + ' '+half +','+half+' '+half * 2+','+half);
        changeSelectorStyle("#grid_to_battery_entity_line",'d', 'M0,'+half+' C'+half+','+ half + ' '+half +','+half+' '+half+','+half * 2);
        changeSelectorStyle('#battery_to_house_entity_line','d', 'M'+ half +','+ half * 2 +' C'+ half +','+ half +' '+ half +','+ half +' '+ half * 2 +','+ half);
        changeSelectorStyle('#generation_to_battery_entity_line', 'd', 'M' + half + ',0 C' + half + ',0 ' + half + ',' + half * 2 + ' ' + half + ',' + half * 2);
