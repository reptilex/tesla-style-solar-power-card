...
import {
  ...
  import { HomeAssistant, fireEvent, LovelaceCardEditor, ActionConfig, LovelaceCardConfig } from 'custom-card-helpers';
  ...
  export class TeslaStyleSolarPowerCardEditor extends LitElement implements LovelaceCardEditor {
    ...
    get _w_not_kw(): boolean {
      return this._config?.w_not_kw || false;
    }
          ...
          ${options.appearance.show
            ? html`
                <div class="values">
                  ...
                  <ha-formfield .label=${`Toggle W instead of kW ${this._w_not_kw ? 'off' : 'on'}`}>
                    <ha-switch
                      .checked=${this._w_not_kw !== false}
                      .configValue=${'w_not_kw'}
                      @change=${this._valueChanged}
                    ></ha-switch>
                  </ha-formfield>
    ...
    private _fillIconEntityMap() {
      if (this._config !== undefined) {
        this._entityMap.set('grid_entity', this._config.home_consumption_entity);
        this._entityMap.set('home_entity', this._config.home_consumption_entity);
        this._entityMap.set('solar_entity', this._config.solar_yield_entity);
        this._entityMap.set('battery_entity', this._config.battery_soc_entity);
        this._entityMap.set('appliance1_entity', this._config.appliance1_state_entity);
        this._entityMap.set('appliance2_entity', this._config.appliance2_state_entity);
      }
    }
