...
describe('TeslaStyleSolarPowerCard with solarConfig', () => {
  ...
  beforeEach(async () => {
    config = {
      ...
      generation_yield_entity: "sensor.generation_yield"
    };
    hass = {
      states: {
        ...
        "sensor.generation_yield": {
          attributes: {
            unit_of_measurement: "W",
          },
          entity_id: "generation_yield",
          state: "8100",
        },
  ...
  it('has generation_yield_entity, text and icon', async () => {
    const solarYieldEntity = teslaCard?.querySelector('.generation_yield_entity')
    if(solarYieldEntity === null || solarYieldEntity === undefined) assert.fail('No generation_yield_entity element found');
    expect(solarYieldEntity?.querySelector('.acc_text')?.innerHTML,"No sum of grid charging flows in acc_text of grid_entity")
      .contains('8.1 kW');
    expect(solarYieldEntity?.querySelector('.acc_icon')?.getAttribute('icon')?.toString())
      .to.equal('mdi:solar-panel-large');
  });

  it('has solar to house feed line and circle', async () => {
    const solarToHouseLine = teslaCard?.querySelector('#generation_to_house_entity_line')
    if(solarToHouseLine === null || solarToHouseLine === undefined) {
      assert.fail('No generation_to_house_entity_line element found');
    }
    const solarToHouseCircle = teslaCard?.querySelector('#generation_to_house_entity_circle')
  ...
  it('has solar to grid line and circle', async () => {
    const solarToGridLine = teslaCard?.querySelector('#generation_to_grid_entity_line')
    if(solarToGridLine === null || solarToGridLine === undefined) {
      assert.fail('No generation_to_grid_entity_line element found');
    }
    const solarToGridCircle = teslaCard?.querySelector('#generation_to_grid_entity_circle')
  ...
  it('has solar to battery line and circle', async () => {
    const solarToBatteryLine = teslaCard?.querySelector('#generation_to_battery_entity_line')
    if(solarToBatteryLine === null || solarToBatteryLine === undefined) {
      assert.fail('No generation_to_battery_entity_line element found');
    }
    const solarToBatteryCircle = teslaCard?.querySelector('#generation_to_battery_entity_circle')
  ...
  it('has no pv, grid or appliance icons', async () => {  
    ...
    if(gridEntity !== null) assert.fail('grid_consumption_entity element found');

    const batteryEntity = teslaCard?.querySelector('.battery_consumption_entity')
    if(batteryEntity !== null) assert.fail('battery_consumption_entity element found');
    
    const houseEntity = teslaCard?.querySelector('.house_consumption_entity')
    if(houseEntity !== null) assert.fail('house_consumption_entity element found');
    
    const appliance1Entity = teslaCard?.querySelector('.appliance1_consumption_entity')
    if(appliance1Entity !== null) assert.fail('appliance1_consumption_entity element found');

    const appliance2Entity = teslaCard?.querySelector('.appliance2_consumption_entity')
    if(appliance2Entity !== null) assert.fail('appliance2_consumption_entity element found');
