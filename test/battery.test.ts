...
describe('TeslaStyleSolarPowerCard battery tests', () => {
  ...
  beforeEach(async () => {
    config = {
      ...
      battery_soc_entity: "sensor.battery_charge",
  ...
  it('has house_consumption_entity, text and icon', async () => {
    ...
    if(houseEntity === null || houseEntity === undefined) assert.fail('No house_consumption_entity element found');
    expect(houseEntity?.querySelector('.acc_text')?.innerHTML)
      .contains('1.3 kW');
    expect(houseEntity?.querySelector('.acc_icon')?.getAttribute('icon')?.toString())
      .to.equal('mdi:home');
  });
  ...
  it('has battery_consumption_entity, text and icon', async () => {
    ...
    if(batteryEntity === null || batteryEntity === undefined) assert.fail('No battery_consumption_entity element found');
    expect(batteryEntity?.querySelector('.acc_text')?.innerHTML)
      .contains('1.3 kW');
    expect(batteryEntity?.querySelector('.acc_icon')?.getAttribute('icon')?.toString())
      .to.equal('mdi:battery');
    expect(batteryEntity?.querySelector('.acc_text_extra')?.innerHTML)
    .contains('100 %');
  });

  it('has battery to house consumption line and circle', async () => {
    // await setCardConsumingFromGrid();
    const batteryToHouseLine = teslaCard?.querySelector('#battery_to_house_entity_line')
    if(batteryToHouseLine === null || batteryToHouseLine === undefined) {
      assert.fail('No battery_to_house_line element found');
    }
    const batteryToHouseCircle = teslaCard?.querySelector('#battery_to_house_entity_circle')
  ...
  it('has no pv, grid or appliance icons', async () => {
    ...
    if(gridEntity !== null) assert.fail('No battery_consumption_entity element found');
