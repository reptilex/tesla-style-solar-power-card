...
describe('TeslaStyleSolarPowerCard with defaultConfig', () => {
  ...
  it('has battery_consumption_entity, text and icon', async () => {
    ...
    expect(batteryEntity?.querySelector('.acc_text')?.innerHTML,"No sum of battery charging flows in acc_text of battery_entity")
      .contains('2 kW');
  ...
  it('has grid to battery charging line and circle', async () => {
    const gridToBatteryLine = teslaCard?.querySelector('#grid_to_battery_entity_line')
    if(gridToBatteryLine === null || gridToBatteryLine === undefined) {
      assert.fail('No grid_to_battery_entity_line element found');
    }
    const gridToBatteryCircle = teslaCard?.querySelector('#grid_to_battery_entity_circle')
