...
describe('TeslaStyleSolarPowerCard with extra appliances', () => {
  ...
  beforeEach(async () => {
    config = {
      ...
      appliance1_state_entity: "sensor.car_soc",
      appliance2_consumption_entity: "sensor.heating_consumption",
      appliance2_state_entity: "sensor.heating_current_function",
    };
    hass = {
      states: {
  ...
  it('has house_consumption_entity, text and icon', async () => {
    ...
    if(gridEntity === null || gridEntity === undefined) assert.fail('No house_consumption_entity element found');
    expect(gridEntity?.querySelector('.acc_text')?.innerHTML)
      .contains('4 kW');
    expect(gridEntity?.querySelector('.acc_icon')?.getAttribute('icon')?.toString())
      .to.equal('mdi:home');
  });

  it('has appliance1_consumption_entity, text and icon', async () => {
    const gridEntity = teslaCard?.querySelector('.appliance1_consumption_entity')
    if(gridEntity === null || gridEntity === undefined) assert.fail('No appliance1_consumption_entity element found');
    expect(gridEntity?.querySelector('.acc_text')?.innerHTML,"No sum of appliance1 charging flows in acc_text of grid_entity")
      .contains('2 kW');
    expect(gridEntity?.querySelector('.acc_icon')?.getAttribute('icon')?.toString())
      .to.equal('mdi:car-sports');
    expect(gridEntity?.querySelector('.acc_text_extra')?.innerHTML,"Appliance 1 extra text is wrong")
    .contains('90 %');
  });

  it('has appliance2_consumption_entity, text and icon', async () => {
    const gridEntity = teslaCard?.querySelector('.appliance2_consumption_entity')
    if(gridEntity === null || gridEntity === undefined) assert.fail('No appliance2_consumption_entity element found');
    expect(gridEntity?.querySelector('.acc_text')?.innerHTML,"No sum of appliance2 charging flows in acc_text of grid_entity")
      .contains('1 kW');
    expect(gridEntity?.querySelector('.acc_icon')?.getAttribute('icon')?.toString())
      .to.equal('mdi:air-filter');
      expect(gridEntity?.querySelector('.acc_text_extra')?.innerHTML,"Appliance 2 extra text is wrong")
      .contains('Warm water');
  });

  it('has appliance1 line and circle', async () => {
    const appliance = teslaCard?.querySelector('#appliance1_consumption_entity_line')
    if(appliance === null || appliance === undefined) {
      assert.fail('No appliance1_consumption_entity_line element found');
    }
    const applianceCircle = teslaCard?.querySelector('#appliance1_consumption_entity_circle')
  ...
  it('has appliance2 line and circle', async () => {
    const applianceLine = teslaCard?.querySelector('#appliance2_consumption_entity_line')
  ...
  it('has no pv, grid or battery icons', async () => {
    ...
    if(pvEntity !== null) assert.fail('pv_consumption_entity element found, should not be there');
    
    const batteryEntity = teslaCard?.querySelector('.battery_consumption_entity')
    if(batteryEntity !== null) assert.fail('battery_consumption_entity element found, should not be there');
    
    const gridEntity = teslaCard?.querySelector('.grid_consumption_entity')
    if(gridEntity !== null) assert.fail('grid_consumption_entity element found, should not be there');
  });
