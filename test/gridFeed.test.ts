import { html, fixture, expect, elementUpdated, assert } from '@open-wc/testing';
...
import {setCard, setCardView, setCardAllInactive} from './setters.js'
...
describe('TeslaStyleSolarPowerCard with defaultConfig', () => {
  ...
  it('has grid_consumption_entity, text and icon', async () => {
    const gridEntity = teslaCard?.querySelector('.grid_consumption_entity')
    if(gridEntity === null || gridEntity === undefined) assert.fail('No grid_consumption_entity element found');
    expect(gridEntity?.querySelector('.acc_text')?.innerHTML,"No sum of grid charging flows in acc_text of grid_entity")
      .contains('2 kW');
  ...
  it('has battery to grid feed in line and circle', async () => {
    const batteryToGridLine = teslaCard?.querySelector('#battery_to_grid_entity_line')
    if(batteryToGridLine === null || batteryToGridLine === undefined) {
      assert.fail('No battery_to_grid_entity_line element found');
    }
    const batteryToGridCircle = teslaCard?.querySelector('#battery_to_grid_entity_circle')
  ...
  it('has no pv, grid or appliance icons', async () => {
    ...
    if(pvEntity !== null) assert.fail('No pv_consumption_entity element found');
    
    const batteryEntity = teslaCard?.querySelector('.battery_consumption_entity')
    if(batteryEntity !== null) assert.fail('No battery_consumption_entity element found');
    
    const houseEntity = teslaCard?.querySelector('.house_consumption_entity')
    if(houseEntity !== null) assert.fail('No house_consumption_entity element found');
    
    const appliance1Entity = teslaCard?.querySelector('.appliance1_consumption_entity')
    if(appliance1Entity !== null) assert.fail('No appliance1_consumption_entity element found');

    const appliance2Entity = teslaCard?.querySelector('.appliance2_consumption_entity')
    if(appliance2Entity !== null) assert.fail('No appliance2_consumption_entity element found');
